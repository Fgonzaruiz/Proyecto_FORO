...
$q = trim($_GET['q'] ?? '');
if ($q === '') {
    $query = $db->query("SELECT id, name FROM {$prefix}game_personajes WHERE name NOT IN ('Narrador', 'STAFF') ORDER BY name ASC LIMIT 50");
} else {
    $escaped = $db->escape_string($q);
    $query = $db->query("SELECT id, name FROM {$prefix}game_personajes WHERE name NOT IN ('Narrador', 'STAFF') AND name LIKE '%{$escaped}%' ORDER BY name ASC LIMIT 20");
}
