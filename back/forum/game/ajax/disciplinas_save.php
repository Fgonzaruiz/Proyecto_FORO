...
if (game_get_active_staff_level($uid) < 3) {
    GameAjax::fail(403, 'No tienes permiso para editar disciplinas');
}
...
$category = trim((string)($input['category'] ?? 'combate'));
$icon = trim((string)($input['icon'] ?? 'fa-crosshairs'));
...
if ($id > 0) {
    $db->write_query("UPDATE {$prefix}game_disciplinas SET
...
$dup = $db->query("SELECT 1 FROM {$prefix}game_disciplinas WHERE slug = '{$escSlug}' LIMIT 1");
if ($db->num_rows($dup)) {
    GameAjax::fail(409, 'Ya existe una disciplina con ese slug');
}

$db->insert_query('game_disciplinas', [
    'slug' => $slug,
    'name' => $name,
